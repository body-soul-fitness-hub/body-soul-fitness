      ...
      <aside className="fixed inset-y-0 left-0 hidden w-[252px] flex-col bg-[#111c19] p-6 text-white lg:flex">
        ...
        <nav className="mt-12 space-y-1">
          {navigation.map(([Icon, label], index) => {
            ...
            return (
              <button
                className={`flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left text-sm font-bold transition ${isActive ? "bg-[#c9f36a] text-[#111c19] shadow-lg shadow-[#c9f36a]/10" : "text-[#bbc5c0] hover:bg-white/8 hover:text-white"}`}
                key={label as string}
              >
                <Icon size={18} />
                {label}
