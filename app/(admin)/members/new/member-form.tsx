...
import { checkMobileExists, createMember } from "@/app/(admin)/members/actions";
import type { FormState } from "@/app/(admin)/members/actions";
import { GENDERS, WORKOUT_TIMES, calculateAge, type Enquiry } from "@/lib/enquiries/types";
...
export function MemberForm({ enquiry }: { enquiry: Enquiry | null }) {
  ...
  const [dob, setDob] = useState(enquiry?.date_of_birth ?? "");
    ...
    <form action={formAction} className="mt-8 max-w-3xl rounded-3xl border border-[#e5e9e5] bg-white p-6 shadow-[0_12px_45px_rgba(17,28,25,.045)] sm:p-8" encType="multipart/form-data">
      ...
      <div className="mt-4 grid gap-5 sm:grid-cols-2">
        ...
        <Field
          ...
          hint={duplicate ? undefined : "10-digit mobile number"}
          label="Mobile number"
          required
        >
          <input className={inputClass} defaultValue={enquiry?.mobile_number ?? ""} name="mobile_number" onBlur={onMobileBlur} required type="tel" />
          {duplicate && (
            <p className="mt-1.5 text-xs font-bold text-[#a94f37]">
              Already registered as <Link className="underline" href={`/members/${duplicate.id}`}>{duplicate.member_id} · {duplicate.full_name}</Link> — view that record instead of creating a duplicate.
            </p>
          )}
        </Field>
        ...
        <Field className="sm:col-span-2" label="WhatsApp messaging">
          <div className="flex flex-col gap-2 sm:flex-row sm:gap-6">
            <label className="flex items-center gap-2 text-sm font-medium normal-case text-[#0f1816]">
              <input className="size-4" name="whatsapp_consent" type="checkbox" />
              Member has consented to WhatsApp messages
            </label>
            <label className="flex items-center gap-2 text-sm font-medium normal-case text-[#0f1816]">
              <input className="size-4" name="whatsapp_promotional_opt_out" type="checkbox" />
      ...
      <div className="mt-4 grid gap-5 sm:grid-cols-2">
        <Field label="Emergency contact name">
          <input className={inputClass} name="emergency_contact_name" type="text" />
        </Field>
        <Field label="Emergency contact number">
          <input className={inputClass} name="emergency_contact_number" type="tel" />
        </Field>
      </div>
      ...
      <div className="mt-4 grid gap-5 sm:grid-cols-2">
        ...
        <Field label="Referred by">
          <input className={inputClass} name="referred_by" type="text" />
        </Field>

        <Field label="Assigned trainer">
          <input className={inputClass} name="assigned_trainer" type="text" />
        </Field>
      ...
      <div className="mt-4">
        <Field label="Any medical conditions, injuries, or health notes the trainer should know about">
          <textarea className={inputClass} name="medical_notes" placeholder="e.g. knee injury, asthma, none declared" rows={3} />
        </Field>
      </div>
      ...
      <div className="mt-8 flex items-center gap-3">
        ...
        <Link className="text-sm font-bold text-[#6c7773]" href={enquiry ? `/enquiries/${enquiry.id}` : "/members"}>Cancel</Link>
      </div>
    </form>
  );
}
