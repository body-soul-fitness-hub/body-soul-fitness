...
export default async function MemberLoginIssuesPage() {
  ...
  return <div className="grid-pattern px-5 py-8 sm:px-8 lg:px-10">
    ...
    <div className="mt-6 overflow-x-auto rounded-3xl border border-[#e5e9e5] bg-white">
      {error ? <p className="p-6 text-sm font-bold text-[#a94f37]">Could not load login events: {error.message}</p> : <table className="w-full min-w-[900px] text-left text-sm"><thead><tr className="border-b border-[#e5e9e5] text-xs uppercase text-[#89938f]"><th className="px-5 py-3">Time</th><th className="px-5 py-3">Member / mobile</th><th className="px-5 py-3">Result</th><th className="px-5 py-3">Reference</th><th className="px-5 py-3">Device</th></tr></thead><tbody>{(data ?? []).map((row: any) => <tr className="border-b border-[#f0f2f0]" key={row.id}><td className="whitespace-nowrap px-5 py-3">{new Intl.DateTimeFormat("en-IN", { timeZone: "Asia/Kolkata", dateStyle: "medium", timeStyle: "medium" }).format(new Date(row.created_at))} IST</td><td className="px-5 py-3"><p className="font-bold">{row.members?.full_name ?? "Unknown member"}</p><p className="text-xs text-[#89938f]">{row.members?.member_id ?? row.mobile_masked}</p></td><td className="px-5 py-3"><span className={row.outcome === "success" ? "inline-flex items-center gap-1 font-bold text-[#577c25]" : "inline-flex items-center gap-1 font-bold text-[#a94f37]"}>{row.outcome === "success" && <CheckCircle2 size={15}/>} {row.auth_error_code === "phone_provider_disabled" ? LABELS.phone_provider_disabled : (LABELS[row.outcome] ?? row.outcome)}</span></td><td className="px-5 py-3 font-mono text-xs text-[#6c7773]">{row.auth_error_code ?? "—"}</td><td className="max-w-xs truncate px-5 py-3 text-xs text-[#6c7773]" title={row.device_summary ?? undefined}>{row.device_summary ?? "—"}</td></tr>)}</tbody></table>}
    </div>
  </div>;
}
