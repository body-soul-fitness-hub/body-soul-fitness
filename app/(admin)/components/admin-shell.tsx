...
export default function AdminShell({ children, name }: { children: React.ReactNode; name: string }) {
  ...
  return <main className="min-h-screen bg-[#f7fbff] text-[#10264a]">
    ...
    <section className="lg:ml-[76px]"><header className="flex h-[82px] items-center justify-between border-b border-[#dceaff] bg-white/90 px-5 backdrop-blur-md sm:px-8 lg:px-10"><div className="flex items-center gap-3 lg:hidden"><button aria-label="Open navigation" onClick={() => setMobileNavOpen(true)} className="grid size-10 place-items-center rounded-xl border border-[#dceaff] bg-white"><Menu size={20}/></button><div className="grid size-10 place-items-center rounded-xl bg-[#2563eb] text-white"><Dumbbell size={20}/></div></div><form action="/members" className="hidden max-w-sm flex-1 items-center gap-3 rounded-xl border border-[#dceaff] bg-[#f7fbff] px-4 py-2.5 focus-within:border-[#2563eb] focus-within:bg-white lg:flex"><Search size={18} className="shrink-0 text-[#6980a5]"/><MemberSearchInput className="w-full bg-transparent text-sm font-medium outline-none placeholder:text-[#8aa0bf]" navigateOnSelect placeholder="Search member name, ID, or mobile…" /><button className="text-xs font-extrabold text-[#2563eb]" type="submit">Search</button></form><div className="ml-auto flex items-center gap-3"><button className="rounded-xl bg-[#2563eb] px-4 py-2.5 text-xs font-extrabold text-white shadow-lg shadow-[#2563eb]/15">AI command center</button><div className="hidden h-9 w-px bg-[#dceaff] sm:block"/><div className="hidden sm:block"><p className="text-sm font-extrabold">{name}</p><p className="text-[11px] font-semibold text-[#6980a5]">Super administrator</p></div><form action={logout} className="lg:hidden"><button aria-label="Sign out" className="grid size-10 place-items-center rounded-xl text-[#6980a5]"><LogOut size={19}/></button></form></div></header>{children}</section>
  </main>;
}
