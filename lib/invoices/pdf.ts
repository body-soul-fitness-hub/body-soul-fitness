...
export type InvoicePdfResult = { buffer: Buffer; filename: string } | null;

// Extracted from the invoice PDF route so both the admin download and the public receipt-link
// download (app/receipt/[token]/pdf) render byte-identical PDFs from one place.
export async function buildInvoicePdf(invoiceId: string): Promise<InvoicePdfResult> {
  const [{ data: invoiceRow, error }, { data: settingsRow }, { data: payments }] = await Promise.all([
    supabaseAdmin.from("invoices").select("*, members(id, member_id, full_name, mobile_number)").eq("id", invoiceId).maybeSingle(),
    supabaseAdmin.from("gym_settings").select("*").eq("id", GYM_SETTINGS_ID).maybeSingle(),
    supabaseAdmin.from("member_payments").select("*").eq("invoice_id", invoiceId).order("payment_date", { ascending: true }),
  ]);
  ...
  return { buffer, filename: `${record.invoice_number}.pdf` };
}
