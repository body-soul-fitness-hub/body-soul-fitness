...
**Latest recorded implementation commit:** `bc442f1`  
**Repository state:** `dev` has been pushed to GitHub. `main` remains unchanged and documentation-only.  
**Supabase projects:** **DEV** = `kuiprqheyynaapncqffa` (wired to `.env.local` and the `dev` Vercel preview branch). **LIVE** = `quznwilwemahoiuoqrpy` (not yet referenced by any app environment variable; schema-only, no real data). Both have `0001_enquiry_management.sql` and `0002_member_management.sql` applied as of v1.6.1.
...
- Both `0001_enquiry_management.sql` and `0002_member_management.sql` have been applied to DEV (`kuiprqheyynaapncqffa`) and LIVE (`quznwilwemahoiuoqrpy`); `SUPABASE_SERVICE_ROLE_KEY` for DEV has been supplied locally in `.env.local`. Neither project's credentials have been added to Vercel's deployment environment variables yet, so the module doesn't work in any hosted deployment (preview or production) until that's done — see §8, "Vercel project import and branch configuration."
- LIVE is schema-ready but has no real data and isn't referenced by any environment variable in the app or in Vercel yet; it exists for when the owner is ready to launch, not for current use.
