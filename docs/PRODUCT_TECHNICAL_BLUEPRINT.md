...
- [-] Implement membership terms, manual payment records, outstanding-balance calculation, and receipt issuance. Subscription terms, manual payment records (`member_payments`), and outstanding-balance (`balance_due`) calculation are done; receipt issuance (PDF/printable) is not yet implemented.
