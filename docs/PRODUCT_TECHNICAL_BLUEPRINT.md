...
**Supabase projects:** **DEV** = `kuiprqheyynaapncqffa` (wired to `.env.local` and Vercel Preview, i.e. the `dev` branch). **LIVE** = `quznwilwemahoiuoqrpy` (wired to Vercel Production, i.e. `main`, though `main` has no application code deployed yet). Both have `0001_enquiry_management.sql` and `0002_member_management.sql` applied and both are currently empty of real data (confirmed as of v1.6.2).
...
- Both `0001_enquiry_management.sql` and `0002_member_management.sql` have been applied to DEV (`kuiprqheyynaapncqffa`) and LIVE (`quznwilwemahoiuoqrpy`); `SUPABASE_SERVICE_ROLE_KEY` for DEV has been supplied locally in `.env.local`. Vercel already has both projects wired in: Preview (`dev` branch) → DEV, Production (`main` branch) → LIVE. `main` has no application code deployed yet, so Production isn't serving the app — merging `dev` into `main` is what will actually put this in front of real traffic against LIVE.
- LIVE is schema-ready and currently holds zero rows in `members`/`enquiries` (a leftover test record from the original v1.5.0 Enquiry Management verification was found and deleted during this check).
