...
| --- | --- | --- | --- |
| 1.9.13 | 2026-08-30 | Checked the `main` deployment for `3bdd3b4` (v1.9.12's Development-target doc commit) via the Vercel CLI, same method as v1.9.3/v1.9.7. Latest Production deployment (`body-soul-fitness-8unh7hg0w-*`) was `● Ready`: clean build log, all 21 routes generated, `Deployment completed`, no errors. | Confirms `main` keeps building cleanly through this run of environment-verification doc commits. |
| 1.9.12 | 2026-08-30 | Checked Vercel's third env target, "Development" (`vercel env pull --environment=development`), completing the matrix started in v1.9.8. Unlike Production and Preview, it has **no Supabase env vars configured at all** — the pulled file contained only Vercel's own auto-generated `VERCEL_OIDC_TOKEN`. This is not a gap: "Development" is the target `vercel dev` reads, and this project's actual local development never uses `vercel dev` — it runs plain `next dev`/`pnpm dev` against `.env.local` directly (DEV credentials, unrelated to Vercel's environment targets). Full matrix as of this entry: Production → LIVE (v1.9.8, re-confirmed v1.9.10); Preview, branch-scoped and unscoped → DEV (v1.9.9, v1.9.11); Development → unconfigured/unused by this project's workflow. Deleted the pulled file (an OIDC token, not a Supabase secret, but consistent with deleting every pulled credentials file immediately after use). | Closes out environment-target verification for this Vercel project — every target has now been checked and either points where documented or is confirmed irrelevant to how the project actually runs, leaving no unverified corner. |
