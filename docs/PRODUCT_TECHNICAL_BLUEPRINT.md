...
**Supabase projects:** **DEV** = `kuiprqheyynaapncqffa` (wired to `.env.local` and Vercel Preview, i.e. the `dev` branch). **LIVE** = `quznwilwemahoiuoqrpy` (wired to Vercel Production, i.e. `main`). Both have `0001_enquiry_management.sql` and `0002_member_management.sql` applied. LIVE serves the live Production deployment (dashboard, Enquiry Management, and — as of v1.7.0 — Member Management), gated by Vercel Deployment Protection rather than in-app auth. DEV backs local development and Preview deployments.
...
- No Supabase authentication or admin/staff login exists yet inside the app itself. Production is currently gated by Vercel Deployment Protection (confirmed: visiting the Production URL redirects to a Vercel account login), which is a meaningful stopgap but is tied to Vercel team membership, not gym staff accounts — it is not a substitute for the planned in-app admin auth, and its exact configuration (which environments/domains it covers) hasn't been audited in Vercel's Settings → Deployment Protection. Add real admin auth and RLS policies before removing Vercel's protection or treating this as safe for unrestricted real member data.
